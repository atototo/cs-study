...
- **현실 비유**: 
    - **나이키 공장**: 나이키 신발 + 나이키 옷 + 나이키 모자 (브랜드 통일)
    - **아디다스 공장**: 아디다스 신발 + 아디다스 옷 + 아디다스 모자
    - *나이키 신발을 신고 아디다스 옷을 입는 '믹스앤매치' 실수를 방지합니다.*
...
**실무적 한계 (Trade-off)**
- **새로운 '종류'의 제품 추가 시 경직성**: 이미 정의된 제품군(버튼, 체크박스) 외에 **새로운 종류의 제품(예: 슬라이더)**을 추가하려면, 최상위 인터페이스인 `AbstractFactory`를 수정해야 합니다. 이는 이를 구현한 **모든 구체적 팩토리 클래스의 수정**을 유발하게 됩니다. 따라서 초기에 제품군의 범위를 신중하게 설계해야 합니다.



| 비교 기준 | 팩토리 메서드 (Factory Method) | 추상 팩토리 (Abstract Factory) |
| :--- | :--- | :--- |
| **생성 대상** | **단일 객체** 생성에 초점 | 연관된 **객체군(Family)** 생성에 초점 |
| **구현 방식** | **상속(Inheritance)** 활용<br/>(서브 클래스에서 생성 메서드 오버라이딩) | **합성(Composition)** 활용<br/>(팩토리 객체를 주입받아 사용) |
...
| **핵심 키워드** | **상속 (Inheritance)**<br/>("자식 클래스야, 네가 결정해") | **합성 (Composition)**<br/>("전용 공장 객체를 끼워줄게") |
