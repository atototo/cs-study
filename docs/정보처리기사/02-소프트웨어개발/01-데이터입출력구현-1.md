...
import DequeOperationImg from '@site/static/img/deque_operation_diagram.png';
...
- **공간 복잡도**
    - **O(N)**: 데이터를 저장하는 만큼 메모리를 사용한다.
    - **특징**: 데이터 자체의 밀도는 높지만, 미리 넉넉하게 할당된 **빈 공간(Capacity)의 낭비**가 발생할 수 있다.
...
- **공간 복잡도**
    - **O(N)**: 데이터를 저장하는 만큼 노드를 생성한다.
    - **특징**: 빈 공간 낭비는 없지만, 각 노드마다 다음 주소를 저장하는 **포인터(링크) 공간이 추가로 필요**하다. (오버헤드 발생)
...
| **공간 효율성** | **높음** (데이터만 저장, 단 미리 할당된 빈 공간 낭비) | **낮음** (데이터 + 포인터 주소값 저장 필요 -> 오버 헤드 발생) |
...
- **시간/공간 복잡도**
    - **시간**: 삽입/삭제 모두 **O(1)** (Top만 건드리면 되므로 매우 빠름)
    - **공간**: **O(N)** (구현 방법에 따라 빈 공간 낭비 또는 포인터 비용 발생)
...
| **공간 효율성** | **높음** (데이터만 저장, 단 빈 공간 낭비 가능) | **낮음** (포인터 저장 오버헤드 발생) |
...
- **시간/공간 복잡도**
    - **시간**: 삽입/삭제 모두 **O(1)** (Front/Rear만 건드리면 됨)
    - **공간**: **O(N)** (선형 큐는 공간 재활용 불가, 원형/연결 큐가 효율적)

#### 큐의 구현 방법 비교
| 구분 | 배열 큐 (Array Queue) | 연결 리스트 큐 (Linked Queue) |
|---|---|---|
| **구조** | 1차원 배열 (주로 원형 큐로 사용) | 연결 리스트 (Front, Rear 포인터) |
| **장점** | 접근 속도가 빠르고 구현이 간단함 | 크기 제한 없음, 삽입/삭제 용이 |
| **단점** | 크기 고정, 선형 큐 사용 시 공간 낭비 심함 | 포인터 저장 오버헤드 발생 |
| **공간 효율성** | **높음** (데이터만 저장) | **낮음** (포인터 때문에 추가 메모리 사용) |
...
### 5. 데크 (Deque)
- **개념**: 큐의 한 유형으로 **양쪽 끝(Front, Rear)에서 삽입/삭제가 모두 가능**한 자료구조 (ADT).
- **특징**: 스택(LIFO)의 성질과 큐(FIFO)의 성질을 모두 가질 수 있는 **만능 자료구조**이다.
- **용도**: 스크롤(Scroll), 문서 편집기의 Undo/Redo, 웹 브라우저 방문 기록, **Java의 스택 대용**

<div style={{ textAlign: 'center', margin: '2rem 0' }}>
  <img 
    src={DequeOperationImg} 
    alt="데크의 동작 원리와 종류(Scroll, Shelf)" 
    style={{ 
      maxWidth: '600px', 
      width: '100%', 
      borderRadius: '20px', 
...
#### 데크의 주요 연산 (ADT)
1. **Push Front / Offer First**: 앞쪽에 데이터 삽입
2. **Push Rear / Offer Last**: 뒤쪽에 데이터 삽입 (일반 큐의 Enqueue)
3. **Pop Front / Poll First**: 앞쪽 데이터 삭제 (일반 큐의 Dequeue)
4. **Pop Rear / Poll Last**: 뒤쪽 데이터 삭제 (스택의 Pop)

#### 데크의 종류 (정보처리기사 단골)
1. **스크롤 (Scroll)**: **입력 제한** 데크
   - 입력은 한쪽에서만 가능, 출력은 양쪽 모두 가능.
2. **쉘프 (Shelf)**: **출력 제한** 데크
   - 출력은 한쪽에서만 가능, 입력은 양쪽 모두 가능.

> **💡 [실무] Java `Deque` (`ArrayDeque`)는 왜 `Stack`보다 좋은가요?**
>
> 1.  **속도 (No Synchronization)**: `Stack`은 모든 메서드에 `synchronized` 락이 걸려 있어 느리지만, `ArrayDeque`는 락이 없어 싱글 스레드 환경에서 훨씬 빠릅니다.
> 2.  **캐시 효율 (Cache Locality)**: `LinkedList` 기반 스택보다 `ArrayDeque`(배열 기반)가 메모리상에 연속적으로 위치하므로 CPU 캐시 적중률(Hit Rate)이 높아 성능이 우수합니다.
>
> **결론**: 스택이 필요하면 무조건 아래처럼 쓰세요!
> ```java
> Deque<String> stack = new ArrayDeque<>();
> stack.push("A");  // 앞으로 넣음 (Stack의 push와 동일)
> stack.pop();      // 앞에서 뺌 (Stack의 pop과 동일)
> ```

---

#### 🚀 [실무 마스터] 스택 vs 큐 vs 데크: 언제 무엇을 쓸까?
헷갈릴 땐 **"데이터가 어디서 들어와서 어디로 나가는지"** 만 생각하세요!

| 자료구조 | 핵심 키워드 | 실제 서비스/기능 예시 | 비고 |
| :--- | :--- | :--- | :--- |
| **스택 (Stack)** | **되돌리기, 후입선출** | • **웹 브라우저 '뒤로 가기'**<br/>• **Ctrl+Z (실행 취소)**<br/>• 모바일 앱 화면 네비게이션 | "방금 한 거 취소해!" |
| **큐 (Queue)** | **순서대로, 대기열** | • **프린터 인쇄 대기열**<br/>• **티켓팅/서버 접속 대기**<br/>• 맛집 줄서기 시스템 (캐치테이블) | "선착순입니다!" |
| **데크 (Deque)** | **유연함, 양쪽 다** | • **최근 검색어 목록** (앞에 넣고 뒤는 삭제)<br/>• **스케줄링** (바쁜 친구 일 뺏어오기)<br/>• **Java의 Stack 대용** | "앞뒤로 다 쓸래!" |
